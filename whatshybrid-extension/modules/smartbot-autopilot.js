...
REGRAS: Responda diretamente, máximo 2-3 frases, português brasileiro.`
  ...
  // REMOVED v7.5.0: Floating panel removed - UI now only in sidepanel
  // ============================================================

  function updateUI() {
    // Emit events for sidepanel to update if available
    emitEvent('ui:update', {
  ...
  function init() {
    ...
    const checkWhatsApp = setInterval(() => {
      ...
      if (chatList) {
        ...
        // v7.5.0: Removed createControlPanel() - UI now only in sidepanel
        
        // Carrega configurações salvas
        loadSavedConfig();
        
        log('✅ Auto-Pilot inicializado (UI removed - use sidepanel)');
  ...
  const AutoPilot = {
    ...
    // UI - Removed v7.5.0: Panel now only in sidepanel
    updateUI,
