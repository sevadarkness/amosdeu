...
(function initAutoPilotCard() {
    ...
    function setupAutoPilotCard() {
        ...
        if (!startBtn) {
            ...
            console.warn('[SidePanel] Elementos do Auto-Pilot não encontrados');
            return;
        }
      ...
      return `
        <div class="recover-item" data-id="${msg.id}" style="padding:10px;border-bottom:1px solid rgba(255,255,255,0.1)">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:11px;color:#8696a0">${date}</span>
            <span>${icon} ${(msg.action || 'msg').toUpperCase()}</span>
          </div>
          <div style="margin-top:4px;font-size:12px;color:#667781">
            ${msg.from || '?'} → ${msg.to || '?'}
          </div>
          <div style="margin-top:4px">${(msg.body || '[mídia]').substring(0, 150)}</div>
          <div style="margin-top:6px;display:flex;gap:6px">
            <button class="recover-fav-btn sp-btn sp-btn-sm" data-id="${msg.id}">${isFav ? '⭐' : '☆'} Favorito</button>
            ${msg.action === 'edited' ? `<button class="recover-compare-btn sp-btn sp-btn-sm" data-id="${msg.id}">📊 Comparar</button>` : ''}
          </div>
        </div>
      `;
    }).join('');
        ...
        if (e.target.checked) {
          Notification.requestPermission().then(p => {
            if (p === 'granted') {
              ...
              alert('Notificações ativadas para todas as mensagens recuperadas');
            } else {
